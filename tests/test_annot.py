...
def test_to_df_noempty(test_events):
    ...
    print(df.columns)
    assert tuple(df.columns) == ("name", "start_seconds", "stop_seconds", "channel")


def test_to_lists(test_events):
    names, starts, stops, channels = test_events.to_lists()
    assert len(names) == 22
...
def test_update_channels():
    evt_main = Events(data={"sine": [[1, 2, 10], [3, 4, 12]], "pulse": [[5, 5], [6, 6]], "vibration": [25, 25]})
    evt_new = Events(data={"sine": [[1, 2, 13], [3, 4, 13]], "vibration": [[7, 7], [8, 8]], "whatnew": [[10, 10], [18, 18]]})
    evt_main.update(evt_new)
    assert len(evt_main.names) == 4
    assert evt_main["vibration"].shape[0] == 2
