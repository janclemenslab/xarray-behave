...
class Poses:
    def make(self, filename: Optional[str] = None):
        ...
        xr_poses = xr.DataArray(
            data=poses,
            dims=["frame_number", "flies", "poseparts", "coords"],
            coords={
                "frame_number": np.arange(first_pose_frame, last_pose_frame),
...
class DeepPoseKit(Poses, io.BaseProvider):
    ...
    def load(self, filename: Optional[str] = None):
        ...
        head_thorax_angle = (
            270
            + np.arctan2(poses_ego.sel(poseparts="head", coords="y"), poses_ego.sel(poseparts="head", coords="x")) * 180 / np.pi
        )
