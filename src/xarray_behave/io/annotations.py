...
class DAS(io.BaseProvider):
    ...
    def load(self, filename: Optional[str] = None):
        ...
        if "event_indices" in res or "segment_labels" in res:  # load old style format
            ...
            for segment_name, segment_labels in zip(res["segment_names"], res["segment_labels"]):
                ...
                res["segment_offsets_seconds"] = np.append(
                    res["segment_offsets_seconds"], segment_offset_idx / res["samplerate_Hz"]
                )
                res["segment_sequence"] = np.append(
                    res["segment_sequence"], [segment_name for _ in range(len(segment_onset_idx))]
                )
        ...
        for segment_onsets, segment_offsets, segment_names in zip(
            res["segment_onsets_seconds"], res["segment_offsets_seconds"], res["segment_sequence"]
        ):
...
class FlySongSegmenter(io.BaseProvider):
    ...
    def load(self, filename: Optional[str] = None):
        ...
        event_seconds = {
            "song_pulse_any_fss": res["pulse_times_samples"] / fs,
